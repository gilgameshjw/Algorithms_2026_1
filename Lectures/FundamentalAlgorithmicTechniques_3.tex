...
\vspace{1em}
	...
	\textbf{Complexity:} $T(n) = 2T(n/2) + cn$ \\ $\mathbf \Longrightarrow \mathcal{O}(nlog(n))$
...
\begin{enumerate}
    ...
    If $ f(n) = \mathcal{O}(n^c) $ where $ c < c_{\text{crit}} $, then:
    \[
    T(n) = \mathcal{O}(n^{\log_b a})
    \]

    \item \textbf{Case 2 (Balanced):} \\
    If $ f(n) = \mathcal{O}(n^{c_{\text{crit}}}) $, then:
    \[
    T(n) = \mathcal{O}(n^{c_{\text{crit}}} \log n) = \mathcal{O}(n^{\log_b a} \log n)
    \]

    \item \textbf{Case 3 (Work Dominated):} \\
    If $ f(n) = \mathcal{O}(n^c) $ where $ c > c_{\text{crit}} $, and if the \textbf{regularity condition} holds:
    \[
