...
\footnotesize  % ← Added here

\textcolor{new_turquoise}{\bf It's Graph Traversal on a Grid}\\
Pixels = nodes; edges = 4 neighbors.\\
Goal: find the same-color \textit{connected component}.

\vspace{0.2cm}
\textcolor{new_turquoise}{\bf Why Avoid Recursion?}\\
Recursion uses the limited \textbf{call stack}.\\
1M pixels = 1M nested calls →  \texttt{StackOverflow!!!}.\\
\textbf{Iterative} (explicit queue/stack) → heap memory → safe at any scale.

\vspace{0.2cm}
\textcolor{new_turquoise}{\bf BFS vs DFS: Shape Matters!}
\begin{itemize}
    \item \textbf{BFS (Queue)}: Smooth, circular fill \textit{(Photoshop default)}
    \item \textbf{DFS (Stack)}: Deep, jagged fill \textit{(fast for small spots)}
\end{itemize}

\vspace{0.2cm}
\textcolor{new_turquoise}{\bf Real-World Uses}
\begin{itemize}
    \item \textbf{Medical}: Tumor/organ segmentation
    \item \textbf{AI Vision}: Region-growing object detection
    \item \textbf{Games}: Territory control \& fog-of-war
\end{itemize}

\vspace{0.2cm}
{\footnotesize \textcolor{gray}{Ref:} \href{https://en.wikipedia.org/wiki/Flood_fill}{\texttt{en.wikipedia.org/wiki/Flood\_fill}}}
\end{frame}
